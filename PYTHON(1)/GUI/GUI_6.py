...
def view():
    ...
    if var.get() == 1:  # Filtering by date range
        ...
        sql = "SELECT DateTime, [User], [Operational Shift], [Station Name], [Process Name], [Battery ID], [Glue Weight], [Cycle Weight] FROM TAPR102_1.dbo.Table_1 WHERE DateTime BETWEEN ? AND ?"
        ...
        sql =  'SELECT DateTime, [User], [Operational Shift], [Station Name], [Process Name], [Battery ID], [Glue Weight], [Cycle Weight] FROM TAPR102_1.dbo.Table_1'
...
def savefile():
    ...
    df = pd.DataFrame(data, columns=['DateTime', '[User]', '[Operational Shift]', '[Station Name]', '[Process Name]', '[Battery ID]', '[Glue Weight]', '[Cycle Weight]'])
...
# ---------------------------------------------------------------------------------------------------------
table = ttk.Treeview(table_frame, columns=('DateTime', '[User]', '[Operational Shift]', '[Station Name]', '[Process Name]', '[Battery ID]', '[Glue Weight]', '[Cycle Weight]'), show="headings", height=1000)
...
table.column("[Cycle Weight]", anchor=tk.CENTER, width=100)
